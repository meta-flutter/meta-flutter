...
def stale_pub_cache_fragment(root):
    """flutter-sdk-pubcache.inc generated for a different FLUTTER_SDK_TAG.

    The fragment is generated by tools/pubvendor from the SDK archive's own
    flutter/pubspec.lock, so it is only valid for the tag it was generated
    from. A roll that bumps FLUTTER_SDK_TAG without regenerating it stages the
    previous release's packages. flutter-sdk_git.bb catches that at build time
    by checking PUBSPEC_LOCK_SHA256 against the archive, but the tag is right
    here in the tree and costs nothing to compare.
    """
    fragment = os.path.join(root, 'recipes-graphics', 'flutter-sdk',
                            'flutter-sdk-pubcache.inc')
    version_inc = os.path.join(root, 'conf', 'include', 'flutter-version.inc')
    if not os.path.exists(fragment) or not os.path.exists(version_inc):
        return []

    m = re.search(r'FLUTTER_SDK_TAG \?\?=\s*"([^"]+)"', _read(version_inc))
    if not m:
        return []
    tag = m.group(1)

    text = _read(fragment)
    provenance = [l for l in text.splitlines() if l.startswith('# Lockfile:')]
    if not provenance:
        return ['%s: no "# Lockfile:" provenance line; regenerate with '
                'tools/pubvendor so the tag it was built from is recorded'
                % os.path.relpath(fragment, root)]
    if tag not in provenance[0]:
        return ['%s: generated from %r but FLUTTER_SDK_TAG is %s; regenerate '
                'with tools/pubvendor'
                % (os.path.relpath(fragment, root),
                   provenance[0].split('Lockfile:')[1].strip(), tag)]
    return []
...
CHECKS = (
    ...
    ('stale pub cache fragment', stale_pub_cache_fragment,
     'flutter-sdk-pubcache.inc was generated for a different FLUTTER_SDK_TAG'),
)
